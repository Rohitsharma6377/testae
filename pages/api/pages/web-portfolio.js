...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      const web_portfolio = await query(`SELECT web_portfolio.*, clients.name AS client_name, websites.url AS website_url, websites.media_id, media.* FROM web_portfolio LEFT JOIN websites ON web_portfolio.website_id = websites.id LEFT JOIN media ON websites.media_id = media.id LEFT JOIN clients ON websites.client_id = clients.id`
      );

      const websites = await query('SELECT websites.*, media.* FROM websites LEFT JOIN media ON websites.media_id = media.id')

      res.status(200).json({
        web_portfolio, websites
      });
    } catch (error) {
  ...
  // add web-portfolio
    ...
    // saveToDB
      try {
        const sql = 'INSERT INTO web_portfolio (website_id, text, status, display_order) VALUES (?, ?, ?, ?)';
  ...
  // update web-portfolio
    ...
    // saveToDB
    try {
      const sql = 'UPDATE web_portfolio SET website_id=?, text=?, status=?, display_order=? WHERE id=?';
