  ...
  if (req.method === 'POST') {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      try {
        ...
        const mediaResponse = await query('INSERT INTO `media`(`media`, `alt`, `path`) VALUES (?, ?, ?)', [fileName, `Portfolio ${name}`, image_path]);
        mediaInsertedId = mediaResponse.insertId;

        const sql = 'INSERT INTO portfolio (name, url, description, display_order, status, media_id) VALUES (?, ?, ?, ?, ?, ?)';
