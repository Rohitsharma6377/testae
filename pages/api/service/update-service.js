  ...
  if (req.method === 'PUT') {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      const uploadFile = () => {
        ...
        fileName = slugify(`service ${name}`) + '.' + files.image[0].originalFilename.split('.')[1];

        let newPath = path.join(process.cwd(), 'public/uploads/services') + '/' + fileName;
      ...
      try {
        await query('UPDATE media SET media = ?, alt = ?, path = ? WHERE id = ?', [fileName ? fileName : image_path.split('uploads/services/')[1], `Service ${name}`, image_path, media_id]);

        let sql = 'UPDATE services SET name = ?, url = ?, description = ?, display_order = ?, status = ?, media_id = ? WHERE id = ?';
