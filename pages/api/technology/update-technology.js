...
export default async function handler(req, res) {
  if (req.method === 'PUT') {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      try {
        await query('UPDATE media SET media = ?, alt = ?, path = ? WHERE id = ?', [fileName ? fileName : image_path.split('uploads/technology/')[1], `Technology ${name}`, image_path, media_id]);

        let sql = 'UPDATE technology SET name = ?, url = ?, description = ?, display_order = ?, status = ?, media_id = ? WHERE id = ?';
