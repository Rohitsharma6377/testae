  ...
  return (
    <div className="portfolio-item-container grid grid-cols-1 md:grid-cols-6 border border-gray-300 rounded-xl overflow-hidden mt-6">
      ...
      <div className="p-8 md:col-span-4">
        ...
        <div className="leading-7 text-sm mb-6" dangerouslySetInnerHTML={{ __html: text }}></div>
        <a href={`${baseUrl}/${path}`} target='_blank' className='bg-[#00203f] hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-full focus:outline-none'>Check Design</a>
      </div>
    </div>
  );
};
...
const PortfolioSection = ({ data }) => {
  data = data.ui_portfolio.filter(item => parseInt(item.status) === 1);

  return (
    <div className="mt-32">
      <Head>
        <title>{'UI Portfolio | AmitKK'}</title>
      ...
      <div className="px-4 md:px-20 mt-14 mb-10">
        <h1 className="text-3xl font-bold text-center text-black mb-2">UI Portfolio</h1>
...
export async function getServerSideProps() {
  const res = await fetch(`${baseUrl}/api/pages/ui-portfolio`);
