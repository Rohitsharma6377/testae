  ...
  return (
    <>
      ...
      <section className='text-center bg-[#F4EBD0] px-10 md:px-32 py-20'>
        ...
        <div className="text-gray-600  mx-auto leading-relaxed" dangerouslySetInnerHTML={{ __html: points[2][0].text }}>
        </div>
      ...
      <section className='px-10 md:px-32 py-20'>
          <div className="grid grid-cols-1 lg:grid-cols-3 text-center lg:text-left justify-between gap-10 items-center">
            ...
            <Image loader={() => points[4][0].media_path} src={`${baseUrl}/${points[4][0].media_path}`} width={250} height={250} alt={points[4][0].media_alt} className="" />
            </div>

            <div className="lg:col-span-2 mx-auto">
              <h3 className="font-bold text-3xl text-gray-800 mb-4">{points[4][0].para}</h3>
              <div className="text-gray-600 mb-6 text-justify" dangerouslySetInnerHTML={{__html: points[4][0].text}}></div>
      ...
      <section className="text-center bg-[#F4EBD0] px-10 md:px-32 py-20">
        ...
        <div className="mt-12 w-full grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 text-left mx-auto gap-10">
            {points[6].map((item, index) => (
              <div key={index} className="bg-white rounded-lg p-8 border  overflow-hidden">
                ...
                <h3 className="text-2xl text-gray-800 mb-2 text-center lg:text-left font-bold">{item.para}</h3>
                <div className="text-gray-600 text-justify" dangerouslySetInnerHTML={{__html: item.text}}></div>
              </div>
            ))}
        </div>
      ...
      <section className='px-10 md:px-32 py-20'>
          <div className="grid grid-cols-1 lg:grid-cols-3 text-center lg:text-left justify-between gap-10 items-center">
            <div className="my-auto max-lg:mx-auto lg:mr-auto">
              <Image loader={() => points[7][0].media_path} src={`${baseUrl}/${points[7][0].media_path}`} width={250} height={250} alt={points[7][0].media_alt} className="" />
            </div>

            <div className="lg:col-span-2 mx-auto">
              <h3 className="font-bold text-3xl text-gray-800 mb-4">{points[7][0].para}</h3>
              <div className="text-gray-600 text-justify mb-6" dangerouslySetInnerHTML={{ __html: points[7][0].text}}></div>
              <button className="font-bold bg-red-600 text-white rounded-lg py-3 px-6 hover:bg-red-700 transition duration-200">Know More</button>
            </div>
          </div>
      </section>
