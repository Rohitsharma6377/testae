            ...
            <div className="w-full flex flex-col gap-4 items-center justify-center rounded-md border h-44 overflow-hidden">
                <div className="relative">
                    <Image
                        ...
                        width={100}
                        height={100}
                        className="mx-auto w-24" // Positions image within container
                    />
                </div>
...
export default function Services({ banners, pages, points, testimonials, faqs, services }){
    const [searchTerm, setSearchTerm] = useState("");
    services = services.filter(service => parseInt(service.status) === 1);

    const filteredData = searchTerm ? services.filter(service => 
        service.name.toLowerCase().includes(searchTerm.toLowerCase())) : services;
...
export async function getServerSideProps(context) {
    ...
    const res = await fetch(`${baseUrl}/api/service/get-service`);
