...
export default function Block9 ({ data }) {
    return (
        <div className="mx-auto text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
                {data.map((item, index) => (
                <div key={index} className={`col-span-1 group relative w-full rounded-2xl p-6 hover:-translate-y-3 transition-all duration-700 ease-in-out hover:bg-indigo-600 ${(index === 1 || index === 2) && 'bg-indigo-600 text-white'}`}>
                    <div className={`bg-indigo-700 glass ease-in-out rounded-xl flex justify-center items-center mb-5 w-14 h-14 p-4 ${(index === 1 || index === 2) && 'bg-white'}`}>
                        <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path className={`${(index === 1 || index === 2) ? 'stroke-white' : 'stroke-black'} group-hover:stroke-white transition-all duration-700 ease-in-out`} d="M24.7222 11.6667V7.22225C24.7222 5.99495 23.7273 5 22.5 5H4.72222C3.49492 5 2.5 5.99492 2.5 7.22222V22.7778C2.5 24.0051 3.49492 25 4.72222 25H22.5C23.7273 25 24.7222 24.005 24.7222 22.7777V17.7778M20.8333 17.7778H25.2778C26.5051 17.7778 27.5 16.7829 27.5 15.5556V13.8889C27.5 12.6616 26.5051 11.6667 25.2778 11.6667H20.8333C19.606 11.6667 18.6111 12.6616 18.6111 13.8889V15.5556C18.6111 16.7829 19.606 17.7778 20.8333 17.7778Z" stroke='#fff' stroke-width="2"></path>
                        </svg>
                    </div>
                    <h4 className={`text-left text-xl font-semibold text-gray-800 mb-3 capitalize transition-all duration-700 ease-in-out group-hover:text-white ${(index === 1 || index === 2) && 'text-white group-hover:text-white'}`}>{item.para}</h4>
                    <div className={`text-justify text-sm font-normal text-gray-700 ease-in-out transition-all duration-700 leading-5 group-hover:text-white ${(index === 1 || index === 2) && 'text-white group-hover:text-white'}`} dangerouslySetInnerHTML={{ __html: item.text }}></div>
                </div>
                ))}
            </div>
        </div>
