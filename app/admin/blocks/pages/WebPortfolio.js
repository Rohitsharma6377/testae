                ...
                <tbody className="text-gray-600 text-sm font-light [&>*:nth-child(even)]:bg-gray-200">
                    {dataArray.map((data, index) => (
                        <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td className="py-3 px-6 text-left">
                                <ToggleBtn key={index} table={'web_portfolio'} status={data.status} id={data.id} />
                            </td>
                            <td className="py-3 px-6 text-left">
...
export default function Services() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/api/pages/web-portfolio');
                setServices(response.data.web_portfolio);
    ...
    const handleFormSubmit = async (postData, method) => {
        try {
            const response = await axios[method]('/api/pages/web-portfolio', postData);
