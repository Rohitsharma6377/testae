...
export default function Clients() {
    ...
    function DataTable({ head, dataArray }) {
        const handleEdit = (index) => {
            ...
            setFormData({
                ...
                display_order: selectedClient.display_order,
                media_id: selectedClient.media_id,
                image: selectedClient.image,
                media_path: selectedClient.image
            });
            setShowForm(true);
        };
                ...
                <tbody className="text-gray-600 text-sm font-light [&>*:nth-child(even)]:bg-gray-200">
                    {dataArray.map((data, index) => (
                        <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td className="py-3 px-6 text-left whitespace-nowrap">{data.brand}</td>
                            <td className="py-3 px-6 text-left whitespace-nowrap">{data.name}</td>
                            <td className="py-3 px-6 text-left">{data.role}</td>
                            <td className="py-3 px-6 text-left">
                                {data.image && (
                                    <Image
                                        loader={() => `${process.env.NEXT_PUBLIC_BASE_URL}/${data.image}`}
                            ...
                            <td className="py-3 px-6 text-left">
                                <ToggleBtn key={index} table={"clients"} status={data.status} id={data.id} />
                            </td>
                            <td className="py-3 px-6 text-left">
                                <button onClick={() => handleEdit(index)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2">
                                    Edit
                                </button>
                            </td>
                        </tr>
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("/api/client/get-client");
    ...
    const handleFormSubmit = async (postData, method) => {
        try {
            const apiUrl = method === "post" ? "/api/client/add-client" : "/api/client/update-client";
            ...
            if (formData.id) {
                setClients((clients) => {
                    ...
                    const index = updatedData.findIndex((client) => client.id === formData.id);
    ...
    return (
        <div className="container mx-auto py-5 px-8">
            ...
            {showForm && (
                <div className="fixed py-10 top-0 right-0 h-full w-1/3 bg-white shadow-lg z-50 overflow-y-auto">
                    <div className="flex justify-between items-center px-8 mb-4">
                        ...
                        <AiOutlineClose onClick={closeForm} className="text-gray-600 hover:text-gray-800 cursor-pointer" />
                    </div>
            ...
            <DataTable head={["#", "Brand", "Name", "Role", "Media", "Status", "Action"]} dataArray={clients} />
        </div>
    );
}