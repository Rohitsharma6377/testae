...
export default function Achievements(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/api/pages/achievements');
    ...
    const handleEdit = (index) => {
        const selectedBlogEntry = achievements[index];
    
        setFormData({
            id: selectedBlogEntry.id,
            number: selectedBlogEntry.number,
    ...
    const handleFormSubmit = async (postData, method) => {
        try {
            const response = await axios[method]('/api/pages/achievements', postData);
            ...
            if (formData.id) {
                ...
                setAchievements(item => [...item, response.data.data]);
            }
    ...
    return (
        <div className='container mx-auto py-5 px-8'>
            ...
            {showForm && (
                <div className={`fixed flex flex-col pt-6 right-0 top-0 h-full w-1/3 bg-white shadow-lg z-50 overflow-y-auto`}>
                    ...
                    <div className='flex justify-between items-center px-8 mb-4'>
                        <h2 className="text-lg font-bold">{formData.id ? 'Update Achievement' : 'Add Achievement'}</h2>
            ...
            <DataTable head={['#', 'Number', 'Description', 'Action']} dataArray={achievements} handleEdit={handleEdit}/>
        </div>
    )
}