      ...
      <tbody className="text-gray-600 text-sm font-light [&>*:nth-child(even)]:bg-gray-200">
        {dataArray.map((data, index) => (
          <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
            ...
            <td className="py-3 px-6 text-left">
                <ToggleBtn key={index} table={'portfolio'} status={data.status} id={data.id} />
            </td>
            <td className="py-3 px-6 text-left">
              <button
...
export default function Portfolio() {
  ...
  const handleEdit = (index) => {
    const selectedEntry = portfolio[index];
  ...
  const handleFormSubmit = async (postData, method) => {
    const apiUrl = method === 'put' ? '/api/portfolio/update-portfolio' : '/api/portfolio/add-portfolio';
  ...
  return (
        ...
        <DataTable tableHeadings={['#', 'Name', 'URL', 'Image', 'Status', 'Action']} dataArray={portfolio} handleEdit={handleEdit}/>
