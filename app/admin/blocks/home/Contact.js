...
export default function Contact(){
    ...
    const handleEdit = (index) => {
        const selectedEntry = contact[index];
    ...
    const handleFormSubmit = async (postData, method) => {
        try {
            const response = await axios[method]('/api/contact', postData);
            ...
            if (formData.id) {
                ...
                setContact(item => [...item, response.data.data]);
            }
    ...
    return (
        <div className='container mx-auto py-5 px-8'>
            ...
            {showForm && (
                <div className={`fixed flex flex-col pt-6 right-0 top-0 h-full w-1/3 bg-white shadow-lg z-50 overflow-y-auto`}>
                    ...
                    <div className='flex justify-between items-center px-8 mb-4'>
                        <h2 className="text-lg font-bold">{formData.id ? 'Update' : 'Add'} Contact</h2>
            ...
            <DataTable head={['#', 'Heading', 'Description', 'Action']} dataArray={contact} handleEdit={handleEdit}/>
        </div>
    )
}