      ...
      <tbody className="text-gray-600 text-sm font-light [&>*:nth-child(even)]:bg-gray-200">
        {dataArray.map((data, index) => (
          <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
            ...
            <td className="py-3 px-6 text-left">
                <ToggleBtn key={index} table={'services'} status={data.status} id={data.id} />
            </td>
            <td className="py-3 px-6 text-left">
              <button
...
export default function Services() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/api/service/get-service');
  ...
  const handleFormSubmit = async (postData, method) => {
    const apiUrl = method === 'put' ? '/api/service/update-service' : '/api/service/add-service';
  ...
  return (
        <div className="container mx-auto py-5 px-8">
            ...
            {showForm && (
                ...
                <div className="flex justify-between items-center px-8 mb-4">
                    <h2 className="text-lg font-bold">{formData.id ? 'Update' : 'Add'} Service</h2>
        ...
        <DataTable tableHeadings={['#', 'Name', 'URL', 'Image', 'Status', 'Action']} dataArray={services} handleEdit={handleEdit}/>
